...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${post.title} | Reading Time Blog`,
...
export default async function BlogPostPage({ params }: Props) {
  ...
  const jsonLd = {
    ...
    "author": { "@type": "Organization", "name": "Reading Time Calculator" },
  };
  ...
  const backLabel = post.language === "ko" ? "블로그 목록으로" : post.language === "es" ? "Volver al Blog" : "Back to Blog";
  const ctaTitle = post.language === "ko" ? "지금 글의 읽기 시간을 계산해 보세요!" : post.language === "es" ? "¡Calcula el tiempo de lectura ahora!" : "Calculate Your Article's Reading Time Now!";
  const ctaSubtitle = post.language === "ko" ? "텍스트를 붙여넣으면 즉시 읽기 시간을 알 수 있습니다." : post.language === "es" ? "Pega tu texto y obtén el tiempo de lectura al instante." : "Paste your text and get the reading time instantly.";
  const ctaButton = post.language === "ko" ? "계산기 사용하기" : post.language === "es" ? "Usar calculadora" : "Use Calculator";
        ...
        <article className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 sm:p-14 shadow-[0_20px_50px_rgba(0,0,0,0.05)] dark:shadow-slate-900/50 border border-slate-100 dark:border-slate-800">
          ...
          <div
            className="prose prose-slate dark:prose-invert max-w-none 
              prose-h2:text-2xl prose-h2:font-black prose-h2:mt-12 prose-h2:mb-4 prose-h2:text-slate-900 dark:prose-h2:text-white prose-h2:border-b prose-h2:border-slate-100 dark:prose-h2:border-slate-800 prose-h2:pb-3
              prose-h3:text-xl prose-h3:font-bold prose-h3:mt-8 prose-h3:mb-3 prose-h3:text-indigo-700 dark:prose-h3:text-indigo-400
              prose-p:text-slate-600 dark:prose-p:text-slate-300 prose-p:leading-relaxed prose-p:mb-4
              prose-ul:text-slate-600 dark:prose-ul:text-slate-300 prose-li:mb-1
