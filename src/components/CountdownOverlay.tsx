...
const labels = {
  en: { analyzing: "Analyzing your text…", ready: "Almost ready!", seconds: "s" },
  ko: { analyzing: "텍스트를 분석하고 있습니다…", ready: "거의 완료!", seconds: "초" },
  es: { analyzing: "Analizando tu texto…", ready: "¡Casi listo!", seconds: "s" },
};

export default function CountdownOverlay({ duration = 10, onComplete, lang = "en" }: CountdownOverlayProps) {
  const [remaining, setRemaining] = useState(duration);
  // Ensure we use a valid key for labels
  const t = (labels as any)[lang] || labels.en;
      ...
      <div className="bg-white rounded-3xl shadow-2xl p-10 flex flex-col items-center gap-6 max-w-sm w-full mx-4" style={{ animation: "scaleIn 0.3s ease" }}>
        ...
        <div className="text-center">
          <p className="text-lg font-bold text-slate-800">
            {remaining > 3 ? t.analyzing : t.ready}
          </p>
        </div>
